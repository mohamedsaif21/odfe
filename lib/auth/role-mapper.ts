...
export const ADMIN_NAV_ITEMS: NavItem[] = [
  ...
  { label: "Recipes",          href: "/recipes",           icon: "BookOpen",        roles: ["admin"] },
  { label: "Suppliers",        href: "/suppliers",         icon: "Building2",       roles: ["admin"] },
  { label: "Purchases",        href: "/purchases",         icon: "Truck",           roles: ["admin"] },
  { label: "Loyalty",          href: "/loyalty",           icon: "Award",           roles: ["admin"] },
  ...
  { label: "Payments",         href: "/payments",          icon: "CreditCard",      roles: ["admin", "cashier"] },
...
export async function resolveAuthenticatedProfile(
  ...
  const { data, error } = await supabase
    .from("profiles")
    .select(
      "id, cafe_id, role, full_name, email, avatar_url, is_active"
    )
    .eq("id", userId)
    .single()
  ...
  if (!data.cafe_id) {
    throw new Error("No cafe assigned")
  }
